<template>
  <el-dialog v-model="dialogVisible" title="Report" @open="handleOpen" :close-on-click-modal="false"
    :close-on-press-escape="false" :before-close="handleCloseDialog" :fullscreen="activeStep == 100" id="ReportDialog">
      ...
      <template v-if="activeStep === 2">
        <el-row justify="center">
          <el-col :span="16">
            ...
            <el-scrollbar style="height:calc(100vh - 15vh - 15vh - 54px - 66px - 60px - 16px - 36px);"
              ref="scrollbarRef">
        ...
        <el-row justify="center" style="min-height: 36px;">
          <el-button v-if="updateParameterIsFail" type="primary" plain @click="handleSelectRetry">Retry</el-button>
        </el-row>
      </template>
      ...
      <el-row>
        <el-col style="display: flex;justify-content: space-between;">
          ...
          <div class="toolbar">
            ...
            <el-backtop :right="20" :bottom="40" :visibility-height="10" target="#ReportDialog" />
          </div>
        </el-col>
      </el-row>
      <el-row id="ReportDocument" v-loading="printLoading">
        <el-col v-for="(pageItem, index) in printList" style="margin: 10px 0;" :key="index" class="page-col">
          ...
          <report-main-page :page="pageItem.page" :mode="provideKeys.viewModePrintValue" />
        </el-col>
      </el-row>
    </template>
  </el-dialog>
...
const props = defineProps(['reportDialogVisible']);

const emit = defineEmits(['update:reportDialogVisible', 'closePrintView']);

const dialogVisible = computed({
  get() {
    return props.reportDialogVisible
  },
  set(value) {
    emit('update:reportDialogVisible', value)
  }
})
...
function getFileName() {
  ...
  fileName.value = dasDevices.currentDeviceFileName('Report');
}
...
async function handleSave() {
  ...
  let doc = document.getElementById("ReportDocument");
